...
from public import *
from public.utils.music_utils import pitch2Freq
...
_toneMask = np.array((0, -1, 1, -1, 2, 3, -1, 4, -1, 5, -1, 6))
...
def _getAcciCountText(acciCount: int) -> str:
    if acciCount == 0:
        ...
        return rf"{acciCount}{{\sh}}"
    else:
        return rf"{-acciCount}{{\fl}}"


@lru_cache
def _getAlteredDegrees(acciCount: int) -> np.ndarray:
    if acciCount > 0:
        alteredDegrees = np.arange(6 - acciCount, 6) * 4 % 7
    elif acciCount < 0:
        alteredDegrees = np.arange(-1, -1 - acciCount) * 4 % 7
    else:
        alteredDegrees = np.array((), dtype=int)
    ...
    if co5Order:
        if acciCount > 0:
            for i in range(acciCount):
                mob_text = Tex(r"{\sh}", _noteNamesCo5Order[i], **newLatexConfig(fs=fs))
                mob_text[1].set_color(RED)
                mob_texts.add(mob_text)
            for i in range(acciCount, 7):
                mob_text = Tex("", _noteNamesCo5Order[i], **newLatexConfig(fs=fs))
                mob_texts.add(mob_text)
        else:
            ...
            for i in range(7 + acciCount, 7):
                mob_text = Tex(r"{\fl}", _noteNamesCo5Order[i], **newLatexConfig(fs=fs))
...
def _createKeyNameText(
    ...
    mob_text = Tex(
        (
            f"{ keyNoteNames [ 0 ] } 大调 " f"({ _getAcciCountText ( acciCount ) })"
            if showAcciCount
...
def getIntervalQuality(startDegree: int, degreeDiff: int) -> int:
    toneDiff = (
        scaleDegree2Tone(startDegree + degreeDiff) - scaleDegree2Tone(startDegree)
    ) % 12
    standardToneDiff = _majorScale[degreeDiff % 7]
    if degreeDiff in (0, 3, 4, 7):
        if toneDiff == standardToneDiff:
            return 0  # perfect
...
def getIntervalAcci(
    startDegree: int, degreeDiff: int, semitones: int, startAccidental: int = 0
) -> int:
    actualSemitones = scaleDegree2Tone(startDegree + degreeDiff) - scaleDegree2Tone(
        startDegree
    )
    return semitones - actualSemitones + startAccidental
...
def getDegreeAndAcci(tone: int, acciPref: int = 1) -> tuple[int, int]:
    octave, octaveTone = divmod(tone, 12)
    acciPref = 1 if acciPref >= 0 else -1
    octaveDegree, acci = _toneMask[octaveTone], 0
    if octaveDegree < 0:
        octaveDegree = _toneMask[octaveTone - acciPref]
        acci = acciPref
    return octaveDegree + octave * 7, acci
...
class IntroScene(Scene):
    def construct(self):
        mob_textProvide = Text(
            "提    供", **newTextConfig(fs=2, font="FandolHei")
        ).to_edge(UP, buff=0.75)

        mob_manshiLogo = Tex("M", "athematic", "S", **newLatexConfig(fs=4)).next_to(
            mob_textProvide, DOWN, buff=1.25
        )
        mob_manshiLogo[0].set_color(BLUE_B)
        mob_manshiLogo[-1].set_color(MAROON_A)
        mob_fxpLogo = MathTex(r"\mathbb{F}[x]_p", **newLatexConfig(fs=3)).next_to(
            mob_manshiLogo, DOWN, buff=1.25
        )
        ...
        self.play(
            ...
            mob_manshiLogo[0].animate.move_to(letterMDisplace + manshiLogoCenter),
            mob_manshiLogo[-1].animate.move_to(letterSDisplace + manshiLogoCenter),
            run_time=0.5,
        )
        ...
        VGroup(
            mob_manshiLogo, mob_manshiText, mob_fxpLogo, mob_fxpText
        ).arrange_in_grid(cols=2, buff=(1, 1)).move_to(DOWN * 0.5)
        manshiLogoTargetPoint = mob_manshiLogo.get_center()

        mob_manshiLogo.move_to(manshiLogoCenter)
        self.play(
            mob_manshiLogo.animate.move_to(manshiLogoTargetPoint),
            FadeIn(
                mob_manshiText,
                mob_fxpLogo,
                mob_fxpText,
...
class FrequencyAndIntervalScene(Scene):
    def construct(self):
        ...
        mob_keyboard = (
            MultiOctavePianoKeyboard(octaves=4, **keyboardConfig)
            .center()
            .to_edge(UP, buff=1.5)
        )
        mob_staff = (
            Staff(**staffConfig, staffLength=35)
        ...
        mob_staff.createClef(clefType).setHpos(2)
        self.play(
            FadeIn(
                mob_keyboard,
                mob_staff,
        ...
        def animatePitch(tone: int, acciPref: int = 1):
            ...
            octave = tone // 12
            scaleDegree, acci = getDegreeAndAcci(tone, acciPref)
            # 还原记号不显示
            if acci == 0:
                acci = None
            ...
            noteNameText = f"{loop ( _noteNames, scaleDegree )}$_{{{octave + 4}}}$"
            if acci is not None:
                if acci > 0:
                    noteNameText = r"{\sh}" + noteNameText
                if acci < 0:
                    noteNameText = r"{\fl}" + noteNameText
            mob_noteNameText = Tex(
                noteNameText,
                **newLatexConfig(fs=1.5),
            ).move_to(noteNameTextPosition)
            mob_freqText = Text(f"{freq:.2f} Hz", **newTextConfig(fs=1.5)).move_to(
                freqTextPosition
            )
        ...
        def animateInterval(startDegree: int, degreeDiff: int):
            ...
            mob_scale = mob_staff.createScale(
                ...
                noteheadType=it.chain(
                    ("whole",), it.repeat("black", degreeDiff - 1), ("whole",)
                ),
                buff=1.5,
                add=False,
            ).shiftHpos(10)
            ...
            addMidi(self, startTone + 60)
            addMidi(self, endTone + 60)
            self.wait(2)
            self.play(
                FadeOut(
                    mob_scale[0],
        ...
        self.next_section("pitches")
        pitchesToShow = _majorScale
        for pitch in pitchesToShow:
            animatePitch(pitch)
            self.wait(0.5)
        self.wait(2)
        ...
        self.next_section("octave")
        animateInterval(0, 7)  # 纯八度
        animateInterval(1, 7)  # 纯八度

        # 展示八度的频率比为 2

        self.next_section("octaveFreqRatio")
        mob_longStaff = (
            Staff(**staffConfig, staffLength=70)
            .center()
            .next_to(mob_keyboard, DOWN, buff=1.5)
        )
        ...
        mob_changeClef = (
            # fmt: off
            mob_longStaff
            .createClef("G", change=True, add=False)
            .setHpos(22)
            # fmt: on
        )
        ...
        mob_freqNote = (
            Text("频率 (Hz)", **textConfig, color=YELLOW)
        ...
        for i, mob_note in enumerate(mob_octaves):
            ...
            if mob_lastNote is None:
                addMidi(self, i * 12 + 48, 0.25)
                self.play(
                    FadeIn(mob_freqText, mob_note),
                    mob_keyboard.animate.markKey(i * 12, markColor=MARK_GREEN),
                    run_time=0.5,
                )
                self.wait(1)
            else:
                ...
                mob_mul2Text = (
                    MathTex(r"\times 2", color=RED, **newLatexConfig(fs=1.25))
                    .move_to(mob_arrow.get_midpoint())
                    .shift(UP * 0.1)
                ...
                def _getAnimations():
                    yield from (
                        Transform(mob_tempNote, mob_note),
                    ...
                    if i == 1:
                        yield Create(mob_changeClef)

                addMidi(self, i * 12 + 48, 0.75)
                self.play(*_getAnimations(), run_time=1)
                self.play(Transform(mob_tempFreqText, mob_freqText), run_time=0.5)
                mob_arrows.add(mob_arrow)
                mob_mul2Texts.add(mob_mul2Text)
        ...
        self.add(mob_temp)
        self.play(
            Transform(mob_temp, mob_staff, run_time=1),
            FadeOut(
                mob_octaves,
                mob_freqNote,
                mob_changeClef,
        ...
        self.next_section("otherIntervals")
...
class FrequencyExponentialScene(Scene):
    def construct(self):
        ...
        mob_box = Rectangle(height=5, width=10).set_stroke(width=2).move_to(boxCenter)
        mob_axes = Axes(
            x_length=w,
            y_length=h,
            x_range=(xmin, xmax),
        ...
        mob_graph = mob_axes.plot(pitch2Freq, color=RED).set_z_index(1)
        mob_pitchText = (
            Text("音高 (半音)", **textConfig)
            .align_to(mob_box, DR)
            .shift(0.25 * UL)
        ...
        def createIndication(i, showFreq: bool = False):
            ...
            pointOnX = mob_axes.coords_to_point(x, ymin)
            pointOnY = mob_axes.coords_to_point(xmin, y)
            mob_lineToX = Line(pointOnX, pointOnCurve, stroke_width=2).set_stroke(
                opacity=0.5
            )
            mob_lineToY = Line(pointOnY, pointOnCurve, stroke_width=2).set_stroke(
                opacity=0.5
            )
            mob_pointOnCurve = Dot(pointOnCurve, radius=0.06).set_z_index(2)
            mob_pitchValue = Text(f"{x}", **newTextConfig(fs=1)).next_to(
                pointOnX, DOWN, 0.25
            )
            ...
            if showFreq:
                mob_freqValue = Text(f"{y:.2f}", **newTextConfig(fs=1)).next_to(
                    pointOnY, LEFT, 0.25
                )
        ...
        def updateGraph(mob):
            ...
            mob.become(mob_axes.plot(pitch2Freq, color=RED).set_z_index(1))

        def updateFocusBox(mob):
            newEndpoint = mob_axes.c2p(xFocusMax, yFocusMax)
            reshapeToCorners(mob, mob_box.get_critical_point(DL), newEndpoint)
...
class FrequencyRatioScene(Scene):
    def construct(self):
        ...
        melodyAccidentals = (
            (-1 if newKeyAcciCount < 0 else 1) if i in alteredDegrees else None
        ...
        # mob_textTitle = Tex(
        #     "音程关系 $\\rightarrow$ 频率比", **newLatexConfig(fs=1.25)
        # ).to_corner(UL, buff=0.25)
        mob_frequencyNote = Text(
            "频率 (单位 Hz)", color=YELLOW, **textConfig
        ).to_corner(UR, buff=0.25)

        def createFreqs(freqs):
            mob_noteBlocks = Group(
                *(NoteBlock(size=0.75) for _ in range(len(freqs))),
            )
            mob_freqTexts = VGroup(
                *(Text(f"{freq: 4.2f}", **newTextConfig(fs=1.25)) for freq in freqs),
            )
            Group(*mob_noteBlocks, *mob_freqTexts).arrange_in_grid(rows=2, buff=0.25)
        ...
        def getMelodyAnimations(mob_freqs, tonic: int = 0):
            for mob_noteBlock, mob_text, startDur, tone in zip(
                *mob_freqs, summedNoteDurations, melody
            ):
                delay = startDur * 60 / bpm
                addMidi(self, generateNoteMidi(tone + 72 + tonic), timeOffset=delay)
                yield Succession(
                    Wait(delay),
                    AnimationGroup(
        ...
        mob_freqs2 = createFreqs(newFreqs).move_to(freqs2Center)
        ...
        self.wait(2)
        self.play(FadeOut(mob_freqs1, mob_freqs2, run_time=0.5))

        # 第二部分：谱表展示

        self.next_section("staff")
        # 上方谱表，展示 C 大调的旋律
        mob_staff1 = Staff(**staffConfig, staffLength=staffLength).move_to(staff1Center)
        ...
        # 下方谱表，展示 F 大调的旋律
        mob_staff2 = Staff(**staffConfig, staffLength=staffLength).move_to(staff2Center)
        ...
        for mob_note, tone in zip(mob_melody1, melody):
            ...
            mob_freqText = (
                Text(f"{freq:.2f}", color=YELLOW, **textConfig)
                .rotate(-PI / 4)
        ...
        def _getAnimations():
            for mob_note, tone, mob_oldFreqText in zip(
                mob_melody2, melody, mob_freqTexts1
            ):
        ...
        def getScaleFreqTexts(mob_scale: Scale, startTone: int = 0):
            ...
            for i, mob_note in enumerate(mob_scale):
                mob_freqText = (
                    ...
                    .move_to(mob_note.mob_noteheads.get_center())
                    .next_to(mob_note.parent.mob_staffLines, DOWN, 0.3, coor_mask=UP)
                )
        ...
        mob_keyNameText1 = (
            _createKeyNameText(0, fs=1.25)
            .next_to(mob_staff1, DOWN, buff=0.25)
            .align_to(mob_staff1, LEFT)
        )
        mob_keyNameText2 = (
            _createKeyNameText(newKeyAcciCount, fs=1.25)
...
class SineWaveScene(Scene):
    def construct(self):
        ...
        def getSineFunction(freq: float, amp: float = 1, phi: float = 0):
            return (
                lambda x: amp
                * unitAmplitude
                * np.sin(2 * PI * freq * x / unitWavelength + phi)
            )

        rationalFreqs = (Q(1), Q(5, 4), Q(3, 2))
        chordDegrees = np.array((0, 2, 4))
        chordTones = _majorScale[chordDegrees]
        ...
        regionHeight = totalHeight / (n + 1)
        fillRate = 0.6
        amp = regionHeight * fillRate / 2
        colors = (RED, GREEN, BLUE)
        mixedGraphY = -ry + bottomGap + regionHeight / 2

        def createVline(nPeriods: float) -> Line:
            x = -rx + nPeriods * unitWavelength
            return Line(
                (x, -ry + bottomGap, 0), (x, ry, 0), stroke_width=1, stroke_opacity=0.25
            )
        ...
        funcs = tuple(getSineFunction(freq) for freq in frequencies)

        def createSineGraph(i: int, overflow: float = 0) -> FunctionGraph:
        ...
        def createMixedGraph(
            n: int = len(rationalFreqs), overflow: float = 0
        ) -> FunctionGraph:
            y = -ry + bottomGap + regionHeight / 2
            if n == 0:
                plotFunc = lambda _: 0
            else:
                mixedFunc = lambda x: sum(fn(x) for fn in funcs[:n]) / n
        ...
        def createChord(n: int = n, startTone: int = 0, acciPref: int = 1) -> Chord:
            startDegree, startAcci = getDegreeAndAcci(startTone, acciPref)
            vpos = chordDegrees[:n] + (startDegree - 6)
            mob_chord = (
                mob_staff.createChord(
                    vpos,
                    noteheadType=it.chain(
                        ("whole",),
                        it.repeat("black"),
                    ...
                    accidentals=map(
                        ...
                        (
                            getIntervalAcci(startDegree, degree, semitones, startAcci)
                            for degree, semitones in zip(chordDegrees, chordTones)
                        ),
                    ),
                    add=False,
                )
            ...
            for i, mob_notehead in enumerate(mob_chord.mob_noteheads):
                mob_notehead.set_color(loop(colors, i))
            return mob_chord

        var_stretch = ValueTracker(0)
        mob_graphs = VGroup()
        mob_ratioTexts = VGroup()
        mob_freqTexts = VGroup()

        ratioTextPositions = []
        freqTextPositions = []

        def createFreqText(i: int, position: Point3D):
            def _getFreqText(_=None):
                freqMultiplier = frequencies[i]
                baseFreq = pitch2Freq(var_stretch.get_value() + 60)
                return (
                    Text(
                        f"{baseFreq * freqMultiplier:.2f} Hz",
                        color=YELLOW,
                        **textConfig,
                    )
                    .move_to(position, aligned_edge=RIGHT)
                    .add_background_rectangle(buff=0.1)
                    .set_z_index(3)
                )

            return always_redraw(_getFreqText)

        for i in range(n):
            mob_graph = createSineGraph(i)

            freqFrac = rationalFreqs[i]
            x = rx - 0.5
            y = -ry + bottomGap + (i + 1.5) * regionHeight
            ratioNumber = freqFrac.numerator * lcmDenoms // freqFrac.denominator
            freqTextPosition = np.array((x, y - 0.5, 0))
            ratioTextPosition = np.array((x - 0.2, y + 0.2, 0))

            mob_ratioText = (
                Text(str(ratioNumber), **newTextConfig(fs=2.5))
                .move_to(ratioTextPosition, aligned_edge=RIGHT)
                .add_background_rectangle(buff=0.1)
                .set_z_index(3)
            )
            mob_freqText = createFreqText(i, freqTextPosition)

            mob_graphs.add(mob_graph)
            mob_ratioTexts.add(mob_ratioText)
            mob_freqTexts.add(mob_freqText)

            ratioTextPositions.append(ratioTextPosition)
            freqTextPositions.append(freqTextPosition)

        mob_mixedGraph = createMixedGraph(0)
        mob_mixedGraphBg = (
            Rectangle(
                width=2 * rx,
                height=regionHeight,
            ...
            .shift(UP * mixedGraphY)
            .set_z_index(-1)
        )
        mob_staff = (
            Staff(**staffConfig, staffLength=15)
            .set_y(mixedGraphY)
            .add_background_rectangle(buff=0.25)
            .to_edge(RIGHT, buff=0.25)
            .set_z_index(2)
        )
        mob_staff.createClef("G").setHpos(2).set_z_index(2)
        self.play(
            FadeIn(mob_hlines, mob_vlines, mob_mixedGraphBg, mob_staff, run_time=0.5),
            Create(mob_mixedGraph),
            run_time=0.5,
        )

        mob_chord = None
        baseFreq = pitch2Freq(60)
        for i, (
            ratio,
            mob_graph,
            mob_ratioText,
            ...
            self.play(
                Create(mob_graph),
                FadeIn(mob_ratioText, mob_freqText),
                Transform(mob_mixedGraph, createMixedGraph(i + 1)),
                (
                    Create(mob_chord := createChord(i + 1))
                    if mob_chord is None
                    else Transform(mob_chord, createChord(i + 1))
                ),
                run_time=1,
            )
            self.wait(0.5)

        # 将图像替换成溢出右边界的版本，方便后续缩放操作
        self.remove(*mob_graphs, mob_mixedGraph)
        mob_graphs = VGroup().set_z_index(1)
        for i in range(len(rationalFreqs)):
            mob_graph = createSineGraph(i, overflow=1)
            mob_graphs.add(mob_graph)
        mob_graphs.add(createMixedGraph(overflow=1))
        self.add(mob_graphs)
        self.wait(2)
        ...
        mob_periodRect.stretch_to_fit_width(0).set_x(-rx)
        mob_periodVline = createVline(0).set_stroke(width=2, opacity=1)
        ...
        mob_graphs.save_state()
        mob_periodRect.save_state()
        mob_periodVline.save_state()

        def updateGraphs(mob: VGroup):
            scl = 2 ** (-var_stretch.get_value() / 12)
            mob.restore()
            mob.stretch_about_point(scl, 0, LEFT * rx)

        mob_graphs.add_updater(updateGraphs)
        mob_periodRect.add_updater(updateGraphs)
        mob_periodVline.add_updater(updateGraphs)

        def animateTranspose(tonic: int = 0):
            baseFreq = pitch2Freq(tonic + 60)
            for i, ratio in enumerate(frequencies):
                addWaveform(
                    self,
                    freq=baseFreq * ratio,
                    timeOffset=0.25 * i,
            ...
            self.play(
                var_stretch.animate.set_value(tonic),
                Transform(
                    mob_chord,
                    createChord(startTone=tonic),
                ),
                run_time=1,
            )
            self.wait(2.5)
...
class _12ETScene(Scene):
    def construct(self):
        mob_keyboard = (
            MultiOctavePianoKeyboard(4, **keyboardConfig).center().shift(UP * 0.4)
        )
        targetOctave = 1
        mob_octaveKeyboard: PianoKeyboard = mob_keyboard[targetOctave]
        mob_octaveRect = SurroundingRectangle(
            mob_octaveKeyboard,
            buff=0.1,
            color=BLUE_D,
        )
        mob_octaveText = Text(
            "一个八度",
            **newTextConfig(fs=1.5),
        ).next_to(mob_octaveRect, UP, buff=0.4)

        def createKeyCountText(n: int = 0) -> Text:
            mob_keyCountText = Text(
                f"键数: {n}",
                **newTextConfig(fs=1.25),
            ).next_to(mob_octaveRect, DOWN, buff=0.4)
            return mob_keyCountText

        def createKeyText(i: int) -> Text:
            mob_keyText = mob_keyboard.alignToKey(
                i + 12 * targetOctave,
                withOutlineBackground(Text(str(i), **textConfig)),
                buff=0.2,
            )
            return mob_keyText

        mob_keyCountText = createKeyCountText(0)
        self.play(FadeIn(mob_keyboard))
        self.play(
            Create(mob_octaveRect),
            FadeIn(mob_octaveText),
            run_time=1,
        )
        self.wait(1)

        self.add(mob_keyCountText)
        mob_keyTexts = VGroup()
        for i in range(12):
            addMidi(self, i + 60)
            mob_octaveKeyboard.markKey(i)
            mob_keyText = createKeyText(i)
            mob_keyCountText.become(createKeyCountText(i + 1))
            self.add(mob_keyText)
            mob_keyTexts.add(mob_keyText)
            self.wait(0.5)
        self.wait(1)
        mob_keyText = createKeyText(12)
        mob_keyTexts.add(mob_keyText)

        arrowStartPoint = mob_keyboard.getKey(targetOctave * 12).get_bottom()
        arrowEndPoint = mob_keyboard.getKey((targetOctave + 1) * 12).get_bottom()
        mob_arrow = (
            ArcBetweenPoints(
                arrowStartPoint,
                arrowEndPoint,
        ...
        self.play(
            Transform(
                ...
                Text(
                    "一个八度 = 12 个半音",
                    **newTextConfig(fs=1.5),
                ).next_to(mob_keyboard, UP, buff=1),
            ),
            FadeIn(mob_keyText),
            Create(mob_arrow),
            FadeOut(mob_keyCountText, mob_octaveRect),
            run_time=2,
        )
        self.wait(1)

        mob_ratioFormula = MathTex(
            r"q^{12} = 2",
            **newLatexConfig(fs=1.25),
        ).next_to(mob_keyboard, UP, 1)
        self.play(
            Transform(mob_octaveText, mob_ratioFormula),
            run_time=1,
        )
        self.remove(mob_octaveText)
        self.add(mob_ratioFormula)

        fractionDigits = 25
        ctx = decimal.getcontext()
        ctx.prec = fractionDigits * 2
        _12thRootOf2 = D("2") ** (D("1") / D("12"))

        mob_newRatioFormula = MathTex(
            rf"q = 2^{{1/12}} = ",
            rf"{str ( _12thRootOf2 ) [ : fractionDigits + 2 ] }\dots",
            **newLatexConfig(fs=1.25),
        ).next_to(mob_keyboard, UP, 1)
        mob_12etText = Text("十二平均律", **newTextConfig(fs=1.5)).next_to(
            mob_keyboard, DOWN, buff=1.25
        )
        self.wait(1)
        self.play(
            TransformMatchingShapes(mob_ratioFormula, mob_newRatioFormula),
            FadeOut(mob_octaveText),
            run_time=2,
        )
        self.wait(0.5)
        self.remove(mob_ratioFormula)
        self.add(mob_newRatioFormula)
        self.play(
            Circumscribe(mob_newRatioFormula[1], time_width=3, run_time=2),
            Write(mob_12etText, run_time=1),
        )

        self.wait(2)


class _12ETTableScene(Scene):
    def construct(self):
        intervalNames = (
            "P1",
            "m2",
        ...
        approxTones = np.log2(np.array([float(x) for x in approximations])) * 12
        mob_numberCircle = NumberCircle(maxVal=12)
        mob_numberCircle.createTicks(1)
        mob_numberLabels = mob_numberCircle.addLabels(1, add=False)
        mob_intervalLabels = mob_numberCircle.addLabels(
            1,
            lambda x: Text(intervalNames[x], **textConfig),
            buff=0.1,
            autoRotate=True,
            add=False,
        )
        mob_rays = mob_numberCircle.createRays(1, stroke_width=2, add=False)
        mob_approxRays = VGroup()

        mob_errorArcs = VGroup()
        rotations = np.empty(12, dtype=float)
        for i, (approx, mob_ray) in enumerate(zip(approximations, mob_rays)):
            approxValue = np.log2(float(approx)) * 12
            error = approxValue - i
            rotations[i] = mob_numberCircle.n2a(approxValue) - mob_numberCircle.n2a(i)
            mob_errorArcs.add(
                mob_numberCircle.createArc(
                    i,
                    error,
                    buff=-0.3,
                    stroke_width=32,
                    color=RED if approxValue > i else GREEN,
                    stroke_opacity=0.75,
                )
            )
            mob_approxRay = mob_ray.copy().set_stroke(opacity=0.5)
            mob_approxRays.add(mob_approxRay)

        self.play(
            Create(mob_numberCircle),
            Write(mob_numberLabels),
            LaggedStart(
                *(Create(mob) for mob in mob_rays),
                lag_ratio=0.5,
            ),
            run_time=4,
        )
        self.add(*mob_approxRays)
        self.wait(2)
        self.play(
            Transform(mob_numberLabels, mob_intervalLabels),
            run_time=1,
        )
        self.remove(mob_numberLabels)
        self.add(*mob_intervalLabels)

        mob_ratioFormulas = VGroup()
        for i, approx in enumerate(approximations):
            mob_ratioFormulas.add(
                MathTex(rf"2^{{{i}/12}}", **latexConfig),
                MathTex(
                    rf"\approx {np.pow ( 2, i / 12 ):.6f} \rightarrow", **latexConfig
                ),
                MathTex(
                    frac2Latex(approx, show1=True, small=True),
                    **newLatexConfig(fs=1.5),
                ),
            )
        mob_ratioFormulas[:18].arrange_in_grid(
            cols=3,
            col_alignments="rlc",
            buff=(0.15, 0.3),
        ).to_corner(RIGHT, buff=0.25)
        mob_ratioFormulas[18:].arrange_in_grid(
            cols=3,
            col_alignments="rlc",
            buff=(0.15, 0.3),
            flow_order="ru",
        ).to_corner(LEFT, buff=0.25)
        mob_note = Text(
            "注：此处所取的一组有理数近似值并非唯一选择",
            **textConfig,
        ).to_corner(UL, buff=0.25)
        ...
        self.play(
            AnimationGroup(
                *(
                    AnimationGroup(
                        Create(mob_arc),
                        Rotate(mob_ray, angle, OUT, mob_numberCircle.getPosition()),
                    )
                    for mob_arc, mob_ray, angle in zip(
                        mob_errorArcs, mob_approxRays, rotations
                    )
                ),
            ),
            run_time=1,
        ...
        self.play(
            Write(mob_ratioFormulas, run_time=5),
            FadeIn(mob_note, run_time=0.5),
        )
        self.wait(2)

        # 重新组合公式，将每一行的三个 Mobject 合并为一个 VGroup
        self.remove(mob_ratioFormulas)
        mob_ratioFormulas = VGroup(
            *(VGroup(*mobs) for mobs in it.batched(mob_ratioFormulas, 3))
        )
        self.add(mob_ratioFormulas)

        def getIntervalMidi(tone: int) -> m21.stream.Stream:
            approx = approximations[tone]
            microtone = np.log2(float(approx)) * 1200 - tone * 100
            stream = m21.stream.Stream()
            stream.append(m21.tempo.MetronomeMark(number=60))
            lowerPitch = m21.pitch.Pitch(60)
            upperPitch = m21.pitch.Pitch(60 + tone, microtone=microtone)
            chord = m21.chord.Chord([lowerPitch, upperPitch])
            chord.quarterLength = 2
            stream.append(chord)
            return stream

        def animatePlayInterval(tone: int) -> None:
            mob_glowPoint1 = addGlow(Dot(mob_numberCircle.n2p(0), radius=0.06))
            mob_glowPoint2 = addGlow(
                Dot(mob_numberCircle.n2p(approxTones[tone]), radius=0.06)
            )
            addMidi(self, getIntervalMidi(tone), 0.25)
            mob_intervalLabel = mob_intervalLabels[tone]
            mob_intervalLabel.save_state()
            self.play(
                FadeIn(mob_glowPoint1, mob_glowPoint2, run_time=0.5),
                Circumscribe(
                    mob_ratioFormulas[tone],
                    time_width=3,
                    run_time=2,
                ),
                mob_intervalLabel.animate.scale(1.5).set_color(YELLOW),
            )
            self.wait(2)
            self.play(
                FadeOut(mob_glowPoint1, mob_glowPoint2),
                mob_intervalLabel.animate.restore(),
                run_time=0.5,
            )

        intervals = np.array((3, 4, 5, 7))
        self.play(
            mob_ratioFormulas[interval].animate.set_color(YELLOW)
            for interval in intervals
        )
        self.wait(2)
        for interval in intervals:
            animatePlayInterval(interval)
...
class MajorScaleScene(Scene):
    def construct(self):
        ...
        mob_staff = Staff(**staffConfig, staffLength=35).move_to(staffPosition)
        ...
        def animateSemitoneWholetone(startDegree: int):
            startTone = _majorScale[startDegree]
            semitoneUp = startTone + 1
            wholetoneUp = startTone + 2
            startNoteHpos = 14
            endNoteHpos = 25
            mob_startNote = mob_staff.createNote(
                startDegree - 6, noteheadType="whole", add=False
            ).setHpos(startNoteHpos)
            mob_startNoteNameText = Tex(
                _noteNames[startDegree],
                **newLatexConfig(fs=1.5),
            )
            mob_semitoneArrowText = MathTex(
                r"\xrightarrow{\text{半音}}", **newLatexConfig(1.5)
            )
            mob_wholetoneArrowText = MathTex(
                r"\xrightarrow{\text{全音}}", **newLatexConfig(1.5)
            )

            if majorScaleDiffs[startTone] == 1:
                mob_semitoneUpNote = mob_staff.createNote(
                    startDegree - 5, add=False
                ).setHpos(endNoteHpos)
                mob_wholetoneUpNote = mob_staff.createNote(
                    startDegree - 5, accidental=1, add=False
                ).setHpos(endNoteHpos)
                mob_semitoneUpNoteNameText = Tex(
                    _noteNames[startDegree + 1],
                    color=BLUE,
                    **newLatexConfig(fs=1.5),
                )
                mob_wholetoneUpNoteNameText = Tex(
                    r"{\sh}",
                    _noteNames[startDegree + 1],
                    **newLatexConfig(fs=1.5),
                )
                mob_wholetoneUpNoteNameText[1].set_color(RED)
            else:
                mob_semitoneUpNote = mob_staff.createNote(
                    startDegree - 6, accidental=1, add=False
                ).setHpos(endNoteHpos)
                mob_wholetoneUpNote = mob_staff.createNote(
                    startDegree - 5, add=False
                ).setHpos(endNoteHpos)
                ...
                mob_semitoneUpNoteNameText[1].set_color(BLUE)
                mob_wholetoneUpNoteNameText = Tex(
                    _noteNames[startDegree + 1],
                    color=RED,
                    **newLatexConfig(fs=1.5),
            ...
            mob_semitoneUpNote.mob_noteheads.set_color(BLUE)
            mob_wholetoneUpNote.mob_noteheads.set_color(RED)

            startPoint = mob_numberLine.n2p(startTone)
            semitoneUpPoint = mob_numberLine.n2p(semitoneUp)
            wholetoneUpPoint = mob_numberLine.n2p(wholetoneUp)

            mob_startGlowPoint = addGlow(Dot(startPoint, radius=0.06))
            mob_semitoneUpGlowPoint = addGlow(Dot(semitoneUpPoint, radius=0.06))
            mob_wholetoneUpGlowPoint = addGlow(Dot(wholetoneUpPoint, radius=0.06))

            mob_semitoneUpArrow = ArcBetweenPoints(
                startPoint, semitoneUpPoint, angle=-PI * 2 / 3, color=BLUE
            ).add_tip(**tipConfig)
            mob_wholetoneUpArrow = ArcBetweenPoints(
                startPoint,
                wholetoneUpPoint,
                angle=-PI * 2 / 3,
                color=RED,
            ...
            mob_semitoneNote = Text("半音", **textConfig).next_to(
                mob_numberLine.n2p(startTone + 0.5), DOWN, 0.15
            )
            mob_wholetoneNote = Text("全音", **textConfig).next_to(
                mob_wholetoneUpArrow, UP, 0.1
            )

            mob_g1 = (
                VGroup(
                    mob_startNoteNameText,
                    mob_semitoneArrowText,
                    mob_semitoneUpNoteNameText,
                )
                .arrange_in_grid(rows=1, buff=0.25, row_alignments="d")
                .next_to(mob_staff.mob_staffLines, DOWN, 0.6)
            )

            # 起始音
            addMidi(self, startTone + 60)
            self.play(
                mob_keyboard.animate(run_time=0.25).markKey(
                    startTone, markColor=MARK_GREEN
                ),
                Flash(startPoint),
            )
            self.wait(1)
            self.play(mob_keyboard.animate.resetMarks(), run_time=0.25)

            # 起始音
            addMidi(self, startTone + 60)
            self.add(mob_startNote, mob_startGlowPoint, mob_startNoteNameText)
            mob_keyboard.markKey(startTone, markColor=MARK_GREEN)
            self.wait(0.5)
            self.play(
                Create(mob_semitoneUpArrow), Write(mob_semitoneArrowText), run_time=0.5
            )

            # 高半音的音
            addMidi(self, semitoneUp + 60)
            self.add(
                mob_semitoneUpNote, mob_semitoneUpGlowPoint, mob_semitoneUpNoteNameText
            )
            mob_keyboard.markKey(semitoneUp, markColor=MARK_BLUE)
            self.play(FadeIn(mob_semitoneNote), run_time=0.5)
            self.wait(0.5)
            self.play(
                FadeOut(mob_startGlowPoint, mob_semitoneUpGlowPoint),
                run_time=0.5,
            )
            self.wait(2)
            self.play(
            ...
            mob_g2 = (
                VGroup(
                    mob_startNoteNameText,
                    mob_wholetoneArrowText,
                    mob_wholetoneUpNoteNameText,
                )
                .arrange_in_grid(rows=1, buff=0.25, row_alignments="d")
                .next_to(mob_staff.mob_staffLines, DOWN, 0.6)
            )
            ...
            self.wait(0.5)
            self.play(
                Create(mob_wholetoneUpArrow),
                Write(mob_wholetoneArrowText),
                run_time=0.5,
            )

            # 高全音的音
            addMidi(self, wholetoneUp + 60)
            self.add(
                mob_wholetoneUpNote,
                mob_wholetoneUpGlowPoint,
                mob_wholetoneUpNoteNameText,
            )
            mob_keyboard.markKey(wholetoneUp, markColor=MARK_RED)
            self.play(FadeIn(mob_wholetoneNote), run_time=0.5)
            self.wait(2)
            mob_semitoneUpNote.setHpos((startNoteHpos + endNoteHpos) / 2)
            ...
            mob_g3 = (
                VGroup(
                    mob_startNoteNameText.copy(),
                    mob_semitoneArrowText.copy(),
                    mob_semitoneUpNoteNameText.copy(),
                    mob_semitoneArrowText.copy(),
                    mob_wholetoneUpNoteNameText.copy(),
                )
                .arrange_in_grid(rows=1, buff=0.25, row_alignments="d")
                .next_to(mob_staff.mob_staffLines, DOWN, 0.6)
            )
            ...
            self.play(
                FadeOut(mob_startGlowPoint, mob_wholetoneUpGlowPoint),
                FadeIn(mob_semitoneUpNote, mob_semitoneNote, mob_semitoneUpArrow),
                Transform(mob_g2, mob_g3),
                mob_keyboard.animate.markKey(semitoneUp),
                run_time=0.5,
            )
            self.remove(mob_g2)
            self.add(mob_g3)
        ...
        def animateScaleGeneration(acciCount: int):
            ...
            alteredDegrees = _getAlteredDegrees(acciCount)
            mob_majorNoteNameTexts = _createNoteNameTexts(acciCount).move_to(
                noteTextPosition
            )
            mob_keyNameText = _createKeyNameText(
                acciCount, fs=1.5, showAcciCount=True
            ).move_to(noteTextPosition)
            mob_tonicText = mob_keyboard.alignToKey(
                tonic,
                Text("主\n音", **textConfig).set_stroke(
                    width=4,
                    color=BLACK,
                    background=True,
                    opacity=0.75,
            ...
            mob_indicator = (
                Circle(radius=0.1).set_stroke(width=0).set_fill(color=RED, opacity=1)
            )
            ...
            mob_scale = mob_staff.createScale(
                ...
                accidentals=(
                    None if i not in alteredDegrees else 1 if acciCount > 0 else -1
                    for i in range(7)
                ),
                accidentalSpaceRatio=0,
                add=False,
            ...
            self.play(
                mob_keyboard.animate(run_time=0.25).markKey(tonic, markColor=MARK_RED),
                FadeIn(mob_tonicText, run_time=0.25),
                Flash(mob_numberLine.n2p(0)),
            )
            self.wait(0.5)
            self.play(FadeOut(mob_tonicText, run_time=0.25))
            for i, (tone, diff, mob_text, mob_note) in enumerate(
                zip(_majorScale, majorScaleDiffs, mob_majorNoteNameTexts, mob_scale)
            ):
                ...
                mob_scaleDegree = createScaleDegreeText(i).next_to(p1, DOWN)
                ...
                if i in alteredDegrees:
                    mob_note.mob_noteheads.set_color(RED if acciCount > 0 else GREEN)
                self.add(
                    mob_text,
                    mob_scaleDegree,
                ...
                self.play(
                    var_t.animate(run_time=0.5).set_value(diff),
            ...
            mob_scaleDegree = createScaleDegreeText(0).next_to(
                mob_numberLine.n2p(12), DOWN
            )
            mob_scaleDegreeOnKey = mob_keyboard.alignToKey(
                currentKey,
            ...
            mob_keySig = mob_staff.createKeySig(clefType, acciCount, add=False).after(
                mob_clef, 1.5
            )
            ...
            def _getAnimations():
                for mob_keySigAcci, mob_noteAcci in zip(
                    mob_keySig.accidentals(), lmob_copiedAccidentals
                ):
                    yield mob_noteAcci.animate(run_time=0.5).move_to(mob_keySigAcci)
            ...
            self.play(
                ...
                mob_keyboard.animate(run_time=0.25).resetMarks(),
            )

        animateSemitoneWholetone(3)
        # animateScaleGeneration ( 0 )
        # animateScaleGeneration ( 2 )
        # animateScaleGeneration ( 4 )
...
class KeySignatureScene(Scene):
    def construct(self):
        ...
        acciCountOrder = np.array((0, 7, -5, 2, -3, 4, -1, 6, -6, 1, -4, 3, -2, 5, -7))
        ...
        def createTable(acciCounts, co5Order=False):
            ...
            VGroup(mob_tableLeft, mob_tableRight).arrange_in_grid(
                rows=1, row_alignments="u", buff=1
        ...
        def animateKeySignature(acciCount: int):
            ...
            mob_keyNameText = _createKeyNameText(acciCount).next_to(
                mob_staff.mob_staffLines, RIGHT, 0.6
            )
            mob_majorNoteNameTexts = _createNoteNameTexts(acciCount, buff=0.2).next_to(
                mob_keyNameText, RIGHT, 0.6
            )
            ...
            self.add(mob_keyNameText)
            self.wait(0.25)
            for i, (mob_noteNameText, mob_note) in enumerate(
                zip(mob_majorNoteNameTexts, mob_scale)
            ):
                if i in alteredDegrees:
                    mob_note.mob_noteheads.set_color(RED if acciCount > 0 else GREEN)
            ...
            self.play(
                Transform(mob_keyNameText, getKeyNameTextInTable(acciCount)),
                Transform(mob_majorNoteNameTexts, getNoteNameTextsInTable(acciCount)),
                *_getAnimations(),
        ...
        for acciCount in acciCountOrder:
            animateKeySignature(acciCount)
        ...
        def _getAnimations():
            for (
                ...
                acciCountOrder,
                mob_keyNames,
                mob_tableCo5Order1[::2],
                mob_noteNameGroups,
                mob_tableCo5Order1[1::2],
            ):
        ...
        def _getAnimations():
            for i, j in enumerate(acciCountOrder):
                ...
                yield Transform(
                    mob_tableCo5Order1[i * 2 + 1], mob_tableCo5Order2[2 * k + 1]
                )

        self.play(*_getAnimations())
        self.remove(mob_tableCo5Order1)
        self.add(mob_tableCo5Order2)
        mob_keySigSharps = mob_staff.createKeySig(clefType, 7, add=False).after(
            mob_clef, 1.5
        )
        mob_barline = mob_staff.createBarline(add=False).after(mob_keySigSharps, 6)
        mob_keySigFlats = mob_staff.createKeySig(clefType, -7, add=False).after(
            mob_barline, 1.5
        )
        self.play(
            *(
        ...
        mob_noteNameTextsOnTop = _createNoteNameTexts(
            0, fs=1.5, buff=0.4, co5Order=True
        ).next_to(mob_staff.mob_staffLines, RIGHT, 1.25)
        width = mob_noteNameTextsOnTop.get_width()
        mob_rightArrow = Line(ORIGIN, width * RIGHT, color=RED).next_to(
            mob_noteNameTextsOnTop, UP, 0.25
        )
        mob_rightArrow.add_tip(**tipConfig)
        mob_leftArrow = Line(ORIGIN, width * LEFT, color=GREEN).next_to(
            mob_noteNameTextsOnTop, DOWN, 0.25
        )
        mob_leftArrow.add_tip(**tipConfig)
        mob_sharpSign = MusicGlyph("\ue262", musicFont=musicFont, sp=0.2).toPosition(
            mob_rightArrow.get_critical_point(LEFT) + LEFT * 0.3
        )
        mob_flatSign = MusicGlyph("\ue260", musicFont=musicFont, sp=0.2).toPosition(
            mob_leftArrow.get_critical_point(RIGHT) + RIGHT * 0.3
        )
        ...
        self.play(
            LaggedStart(
                *(Indicate(mob, run_time=0.5) for mob in mob_noteNameTextsOnTop),
                lag_ratio=0.5,
            ),
        )
        self.play(
...
class KeySignatureDisplayScene(Scene):
    def construct(self):
        ...
        def createKeySigStaffWithKeyName(acciCount: int) -> VGroup:
            ...
            mob_keyNameText = _createKeyNameText(acciCount, fs=1).next_to(
                mob_staff.mob_staffLines, DOWN, 0.4
            )
            return VGroup(mob_staff, mob_keyNameText)

        mob_vg = (
            VGroup(*(createKeySigStaffWithKeyName(i) for i in range(-7, 8)))
        ...
        def _getAnimations():
            for i, (mob_staff, mob_keyNameText) in enumerate(mob_vg):
                ...
                mob_keySig = mob_staff.createKeySig(clefType, i - 7, add=False).after(
                    mob_clef, 1.5
                )
...
class SemitoneToCircleOfFifthScene(Scene):
    def construct(self):
        ...
        def createScaleDegreeLabel(i: int, mob: NumberCircle, t: float = 1):
            return mob.addLabel(
                ...
                withOutlineBackground(
                    MathTex(rf"\hat{{{i + 1}}}", **newLatexConfig(fs=1.25)),
                    width=8,
                ),
                INSIDE,
                0.2,
        ...
        mob_numberCircle1 = NumberCircle(maxVal=12).shift(numberCircle1Center)
        ...
        VGroup(mob_text1_1, mob_text1_2, mob_text1_3).move_to(
            mob_numberCircle1.getPosition()
        )
        ...
        for i in range(7):
            ...
            mob_glowPoint = addGlow(Dot(p1, radius=0.06))
            mob_arc = mob_numberCircle1.createArc(currentTone, diff, 0.3).set_stroke(
                color=YELLOW
            )
            mob_glowPoints.add(mob_glowPoint)
            ...
            self.play(
                FadeIn(mob_glowPoint, mob_scaleDegreeText, run_time=0.25),
        ...
        self.play(
            mob_numberCircle2.animate(run_time=1).shift(
                numberCircle2Center - numberCircle1Center
            )
        )
        ...
        VGroup(mob_text2_1, mob_text2_2, mob_text2_3).move_to(
            mob_numberCircle2.getPosition()
        )
        ...
        for i in range(6):
            ...
            mob_glowPoint = addGlow(
                Dot(mob_numberCircle2.n2p(currentTone), radius=0.06)
            )
            mob_arc = mob_numberCircle2.createArc(currentTone, 7, 0.3).set_stroke(
                color=YELLOW
            )
            mob_scaleDegreeText = createScaleDegreeLabel(
                currentDegree, mob_numberCircle2
            )
            mob_glowPoints.add(mob_glowPoint)
            mob_lines2.add(mob_line)
            mob_scaleDegreeTexts2.add(mob_scaleDegreeText)
            self.play(
                FadeIn(mob_scaleDegreeText, mob_glowPoint, run_time=0.25),
            )
            addMidi(self, currentTone + 60)
            self.play(
                Create(mob_line, run_time=0.5),
        ...
        mob_text1 = (
            Text("五度的特殊性", **newTextConfig(fs=2))
            .move_to(textStartPoint)
            .align_to(textStartPoint, UP)
        )
        mob_text2 = Text("十二平均律 (12-TET)", **newTextConfig(fs=1.25)).next_to(
            mob_text1, DOWN, 0.7
        )
        mob_text3 = Text("纯八度 = 12 个半音", **newTextConfig(fs=1.25)).next_to(
            mob_text2, DOWN, 0.35
        )
        mob_text4 = Text("纯五度 =  7 个半音", **newTextConfig(fs=1.25)).next_to(
            mob_text3, DOWN, 0.35
        )
        mob_text5 = MathTex(r"\gcd(12, 7) = 1", **newLatexConfig(fs=1.25)).next_to(
            mob_text4, DOWN, 0.35
        )
        mob_text5[0][4:6].set_color(GREEN)
        mob_text5[0][7:8].set_color(BLUE)
        mob_text6 = Text("以 7 为步长可遍历全部 12 个半音", **textConfig).next_to(
            mob_text5, DOWN, 0.4
        )

        mob_text7 = Text("将八度等比分割为 12 份", **newTextConfig(fs=1.25)).next_to(
            mob_text2, DOWN, 0.35
        )
        mob_text8 = Text(
            "八度频率比 1 : 2",
        ...
        mob_text9 = MathTex(
            r"2^{1/12}", rf"= {np.pow ( 2, 1/12 ):.6f}\dots", **newLatexConfig(fs=1.25)
        )
        mob_text10 = (
            MathTex(
                r"2^{7/12}",
        ...
        VGroup(mob_text9, mob_text10).next_to(mob_text8, DOWN, 0.35)
        ...
        self.play(
            mob_lines2.animate(run_time=0.25).set_stroke(width=2),
        )
        for i, (mob_line, mob_glowPoint) in enumerate(zip(mob_lines2, mob_glowPoints)):
            ...
            self.play(
                ...
                Succession(
                    Wait(0.5),
                    mob_line.animate(run_time=0.25).set_stroke(width=4),
                ),
            )
        ...
        addMidi(self, 65)
        self.play(
            *(FadeOut(mob, run_time=0.5) for mob in mob_glowPoints),
        )
        self.wait(1)
        self.play(
            *(
                mob.animate(run_time=0.25).set_stroke(opacity=0.3)
                for mob in mob_extraLines
            ),
            *(FadeIn(mob, run_time=0.25) for mob in mob_scaleDegreeTexts2),
        )
        ...
        mob_arc = mob_numberCircle2.createArc(0, 1, 0.3).set_stroke(color=YELLOW)
        ...
        mob_arc = mob_numberCircle2.createArc(0, 7, 0.3).set_stroke(color=YELLOW)
        ...
        self.play(
            FadeIn(mob_semitoneCircleText, run_time=0.5),
        )
        ...
        self.play(
            *(
                mob.animate(run_time=1).shift(numberCircle2Center - numberCircle1Center)
                for mob in (
                    mob_numberCircle3,
                    mob_lines3,
        ...
        mob_glowPoint = addGlow(Dot(mob_numberCircle3.n2p(1), radius=0.06))
        mob_arc = mob_numberCircle3.createArc(1, 6, 0.3).set_stroke(color=YELLOW)
        self.play(FadeIn(mob_glowPoint, run_time=0.25))
        ...
        def updateNumberCircle(mob: NumberCircle):
            ...
            mob_ticks = mob[-2]
            t = var_t.get_value()
            for i, (mob_label, mob_tick, mob_line) in enumerate(
                zip(mob_labels, mob_ticks, mob_lines3)
            ):
                val = i * t % 12
        ...
        self.play(
            FadeIn(mob_circleOfFifthText, run_time=0.5),
        )

        # 按照五度圈顺序遍历大调音阶
        for i, mob_scaleDegreeText in zip(range(-1, 6), mob_scaleDegreeTexts3):
            tone = i * 7 % 12
            addMidi(self, tone + 60)
            self.play(
                Indicate(
...
class DividingCircleScene(Scene):
    def construct(self):
        ...
        def createNumberCircle(maxVal: float) -> NumberCircle:
            mob_numberCircle = NumberCircle(maxVal=maxVal).shift(circleCenter)
            mob_ticks = mob_numberCircle.createTicks()
            mob_labels = mob_numberCircle.addLabels(
                1,
                lambda x: TextWithBackground(
                    MathTex(r"\overline{%d}" % x, **latexConfig)
                ).set_z_index(1),
            )
            return mob_numberCircle, mob_ticks, mob_labels

        var_m = ValueTracker(0)
        mob_numberCircle = always_redraw(
            lambda: createNumberCircle(var_m.get_value())[0]
        )
        ...
        self.play(var_m.animate(run_time=2, rate_func=rf.ease_out_cubic).set_value(m))
        ...
        def animateCircleDivision(k: int):
            ...
            for i in range(n if gcdmk == 1 else n + 1):
                mob_modulusText = MathTex(
                    r"{i} \times {{{{ \overline{{{k}}} }}}} = \overline{{{rem}}}".format(
                        i=i, k=k, m=m, rem=i * k % m
                    ),
                    **newLatexConfig(),
                )
            ...
            mob_modulusTexts.arrange_in_grid(
                cols=2, buff=(0.6, 0.4), col_alignments="ll"
            ).next_to(mob_gcdText, DOWN, 0.5).align_to(mob_gcdText, LEFT).shift(
                RIGHT * 1.4
            )
            ...
            if gcdmk == 1:
                ...
                mob_subgroupText[1][: len(str(gcdmk))].set_color(LIGHT_PINK)
                mob_subgroupText[1][-3 - len(str(m)) : -3].set_color(GREEN)

            mob_subgroupText[0][2:-2].set_color(BLUE)

            self.play(Write(mob_gcdText, run_time=0.5))
            self.play(
                mob_groupSymbol.animate(run_time=0.5).set_fill(opacity=0.15),
                Circumscribe(mob_labels[0], time_width=2),
            )

            for current, mob_modulusText in zip(
                np.roll(traverseOrder, -1), mob_modulusTexts
            ):
                ...
                self.play(
                    ShowPassingFlashWithThinningStrokeWidth(
                        mob_arc, time_width=3, run_time=1.5
                    ),
                    Succession(
                        Wait(1),
            ...
            if gcdmk > 1:
                self.play(Write(mob_modulusTexts[-1], run_time=0.5))
            ...
            for i, j in enumerate(np.argsort(traverseOrder)):
                self.play(
                    Indicate(mob_labels[i * gcdmk].original, scale_factor=2),
            ...
            self.play(
                Succession(
                    ...
                    Circumscribe(mob_subgroupText, time_width=2, run_time=1),
                )
            )
            self.wait(2)
...
class DividingOctaveScene(Scene):
    def construct(self):
        ...
        mob_note = Text("注：以下图中采用的坐标轴都是对数轴", **textConfig).to_corner(
            UL, buff=0.25
        )
        mob_tex1 = MathTex(
        ...
        self.play(FadeOut(mob_tex1, mob_tex2, run_time=0.5))
        ...
        def auxNumberLineSetup(mob: NumberLine, direction=UP) -> None:
            ...
            mob.add_labels(getBoundLabels()).add_labels(
                {
                    log2LastRatio: (
                        mob_label := TextWithBackground(
                            MathTex(frac2Latex(lastRatio), **latexConfig)
                        )
                    )
                },
                direction=direction,
        ...
        for i in range(11):
            ...
            mob_mul3 = MathTex(
                "\\times 3", **newLatexConfig(fs=1.25), color=RED
            ...
            mob_auxRatioLabel = TextWithBackground(
                MathTex(frac2Latex(ratio), **latexConfig)
            )
            ...
            mob_div2Texts = VGroup(
                MathTex("\\times 1/2", **newLatexConfig(fs=1.25), color=GREEN).next_to(
                    mob, DOWN, 0.2
                )
            ...
            self.play(
                Succession(
                    *(Create(mob) for mob in mob_arrowBackwards),
                    run_time=1,
                ),
                Succession(
                    *(FadeIn(mob, run_time=0.25) for mob in mob_div2Texts),
                ),
...
class CircleOfFifthScene(Scene):
    def construct(self):
        ...
        mob_circleOfFifth: VGroup = CircleOfFifth().to_corner(UL)
        ...
        mob_keySigIndicator = KeySigIndicator(width=mob_keyboard.get_width()).next_to(
            mob_staff.mob_staffLines, DOWN, 0.75
        )
        ...
        self.play(
            mob_circleOfFifth.var_rotation.animate.set_value(TAU * 3),
            run_time=3,
        )
        self.wait(2)
        ...
        def animateKeyChange(acciCount: int):
            ...
            startDegree = acciCount * 4 % 7
            if acciCount >= 0:
                changedKeys = frozenset(np.arange(6 - acciCount, 6) * 4 % 7)
                keyMarkColor = MARK_RED
                noteMarkColor = RED
            else:
                changedKeys = frozenset(np.arange(-1, -1 - acciCount) * 4 % 7)
            ...
            self.play(
                ...
                # 调号切换
                Transform(mob_keySig, mob_newKeySig, run_time=0.5),
                # 调性名称切换
                Transform(
                    mob_keyNameText,
                ...
                # 升降号数量指示条切换
                mob_keySigIndicator.animate(run_time=0.25).setAcciCount(acciCount),
            )
            ...
            for i, (tone, mob_note) in enumerate(zip(_majorScale, mob_scale)):
                ...
                addMidi(self, midiNote)
                # 高亮五度圈区域
                mob_circleOfFifth.getSectorByKeyAcciAndDegree(acciCount, i).highlight()
                # 钢琴键盘上对应的琴键着色
                # 有变音记号的琴键着特殊颜色 (升号为红色，降号为绿色)
                mob_keyboard.markKey(
                    transposedTone,
                    markColor=(keyMarkColor if i in changedKeys else MARK_BLUE),
                ),
                self.add(mob_note)
                self.wait(0.5)
            self.wait(3)
            ...
            self.play(
                *(FadeOut(mob, run_time=0.25) for mob in mob_scale),
                mob_keyboard.animate(run_time=0.25).resetMarks(),
            )
            self.wait(2)
